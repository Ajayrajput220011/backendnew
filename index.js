...
app.post("/api/orders1", async (req, res) => {
  const {
    first_name, last_name, email, phone, address, pincode,
    subtotal, shipping, tax, total,
    payment_method, payment_status, payment_id,
    cart, name, price, description, category, image
  } = req.body;
  ...
  const values = [
    ...
    subtotal, shipping, tax, total,
    payment_method, payment_status, payment_id,
    JSON.stringify(cart), name, price, description, category, image
  ];
...
app.delete("/api/orders1/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("DELETE FROM orders1 WHERE id = $1", [id]);
    if (result.rowCount > 0) {
...
app.put("/api/orders1/:id", (req, res) => {
  const { id } = req.params;
  const { payment_status } = req.body;
  const sql = "UPDATE orders1 SET payment_status = ? WHERE id = ?";
...
app.put("/api/orders1/:id", async (req, res) => {
  const { id } = req.params;
  const { payment_status } = req.body;
  try {
    await pool.query("UPDATE orders1 SET payment_status = $1 WHERE id = $2", [
      payment_status,
      id,
    ]);
    res.json({ message: "Payment status updated" });
