...
app.get("/api/contacts", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, first_name, last_name, email, message, created_at FROM contacts ORDER BY id Asc"
    );
...
app.get("/api/orders1", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT o.*, 
        COALESCE(json_agg(oi.*) FILTER (WHERE oi.id IS NOT NULL), '[]') ASc items
      FROM orders1 o
...
app.get("/api/orders1", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM orders1 ORDER BY id Asc");
